...
const Form = () => {
	...
	return (
		<div className="row">
			...
			{change_box(InputText.a, InputText.c, InputText.b)}
			<Unfsize data={InputText} />
			<Createlay data={InputText} />
		</div>
	);
};
ReactDOM.render(<Form />, document.getElementById('input'));
const box = () => {
	document.getElementById('layshow').style.display = 'none';
	document.getElementById('boxshow').style.display = 'block';
};
const lay = () => {
	document.getElementById('boxshow').style.display = 'none';
	document.getElementById('layshow').style.display = 'block';
};
const Butt = () => {
	return (
		<div>
			<button id="box" onClick={box}>
				กล่อง
			</button>
			<button id="lay" onClick={lay}>
				กางออก
			</button>
		</div>
	);
};
ReactDOM.render(<Butt />, document.getElementById('butt'));
